...
hold on;
loglog(tau_mean,rho0*hgradM_mean_cntr_batch(idx_ref_Cd,:)/Lx,'o-');
loglog(tau_mean,rho0*hgradM_eddy_cntr_batch(idx_ref_Cd,:)/Lx,'o-');
loglog(tau_mean,rho0*eddyforce_cntr_batch(idx_ref_Cd,:)/Lx,'o-');
loglog(tau_mean,rho0*(hgradM_eddy_cntr_batch(idx_ref_Cd,:)+hgradM_mean_cntr_batch(idx_ref_Cd,:)+eddyforce_cntr_batch(idx_ref_Cd,:))/Lx,'o-');
...
leghandle = legend('Wind stress','Mean IFS','Eddy IFS','Eddy momentum flux convergence','Total eddy force');
set(leghandle,'interpreter','latex','Location','NorthWest');
grid on;

figure(2);
semilogx(tau_mean,Klat_cntr_batch(idx_ref_Cd,:));